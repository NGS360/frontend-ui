...
function RouteComponent() {
  ...
  const { mutate: updateSetting, isPending } = useMutation({
    ...
    onError: (err: AxiosError<HttpValidationError>) => {
      const message = err.response?.data.detail?.toString()
    ...
    return (
      <div className='flex flex-col gap-6'>
        <div className='flex flex-col gap-2'>
          <h1 className="text-3xl">Cloud Configuration</h1>
          <p className="text-muted-foreground">
            Configure cloud storage and infrastructure settings for NGS360.
          </p>
        </div>
        <div className="border rounded-lg p-6">
          <p className="text-muted-foreground">No cloud configuration settings found.</p>
        </div>
      </div>
    )
  }
      ...
      <div className='grid gap-4'>
        {settings.map((setting) => (
          <SettingCard 
            ...
            isPending={isPending}
            onSave={handleSave}
            inputType={setting.key === 'AWS_SECRET_ACCESS_KEY' ? 'password' : 'text'}
          />
        ))}
      </div>
    </div>
